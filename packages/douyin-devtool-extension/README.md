...
构建后的 `dist/extension.js` 是扩展入口。真实 DevTool 加载实验必须使用隔离的用户数据和扩展目录，并单独记录 DevTool 版本、启动参数、人工干预和结果。
...
隔离配置无法越过 DevTool 登录前置页时，可由用户显式设置 `GAMEFORGE_DOUYIN_SMOKE_USE_EXISTING_PROFILE=1`，复用当前 DevTool 配置并以 development extension 临时加载；该模式不执行持久安装，但可能更新 DevTool 的最近项目记录。
...
Runtime action 仅允许 `reload`、视口内单点 `tap`、截图摘要和最多 5 秒的 `collectConsole`。截图只返回字节数与 SHA-256，日志最多 64 条且每条截断到 512 字符；协议不接受任意 JavaScript、拖动、文件访问或远程平台操作。`reload` 与 `tap` 会改变模拟器状态，应由控制端在测试步骤中显式调用。
...
探针优先使用显式 `GAMEFORGE_DOUYIN_CDP_PORT`，否则读取 DevTool 用户数据目录中的 `DevToolsActivePort` 第一行，动态发现本次启动的 loopback CDP 端口。随后选择 `MiniApp Webview`，验证 `tt`、`GameGlobal` 和 Canvas 执行上下文，并对 `Page.captureScreenshot` 结果只输出字节数与 SHA-256。它不读取 `DevToolsActivePort` 的第二行连接标识，不保存截图、不注入游戏代码，也不调用 preview、上传、提审或发布；点击/重载只通过上述固定 action 且不在该 smoke 命令中自动执行。
