...
const server = createServer(async (request, response) => {
  try { const body = await readRequest(request); const { command, value } = body;
    ...
    // Same-URL navigation is a deliberate no-op (avoids ERR_ABORTED from a
    // repeated goto); folding that check into the else-if chain made the
    // no-op case fall through to the unknown-command rejection.
    else if (command === "navigate") { if (page!.url() !== value) await page!.goto(value, { waitUntil: "domcontentloaded" }); } else if (command === "click") await page!.locator(value).click(); else if (command === "fill") await page!.locator(value.selector).fill(value.text); else if (command === "press") await page!.locator(value.selector).press(value.key); else if (command === "waitFor") await page!.locator(value.selector).waitFor({ state: value.state, timeout: value.timeoutMs }); else if (command !== "snapshot") throw new Error(`Playwright helper received an unknown command: ${command}`);
