...
启动器只在显式设置 `GAMEFORGE_LAYAIR_CLI` 时注入该变量；未设置时不猜测用户目录或 PATH，显式空白则 fail-closed。动态配置先拒绝相对路径、目录、缺失文件和符号链接，MCP 启动后再验证版本必须精确为 3.4.0。该入口只供受限的抖音/微信构建工具使用，不开放任意命令参数。
