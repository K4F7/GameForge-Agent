...
> 小游戏状态说明（2026-07-28）：本文中的抖音/微信 target、构建、校验、CLI 和玩法验收段落是截至 2026-07-18 的历史实现记录，不再作为当前命令或 MCP 工具指南。当前主动产品 target 只有 Phaser/Vite Web；现行入口以 [CodeArts 快速开始](codearts-quickstart.md) 为准。
>
...
update 在写任何临时模板文件前创建 0600、128 KiB 上限的严格 `.gameforge/update.transaction.json`。日志只接受最多 30 个规范受管路径及 add/update/delete 动作，记录旧/新文件元数据和旧/新托管 Manifest 精确 SHA-256；临时/备份路径由 transaction UUID 推导。托管 Manifest 是提交点：`recover_game_project_update` 持锁后发现旧 Manifest 时逆序恢复备份并删除匹配的新文件，发现新 Manifest 时验证新文件并完成清理；既非旧也非新、任意哈希不符、符号链接或未知字段都会保留现场并报错。`generate_game_project` 不会隐式恢复，恢复后必须重新 dry-run 获取新 CAS。
...
首次创建和替换都先以 0600 文件写入严格事务日志，再写新媒体与新 Manifest 的同目录临时文件。create 以硬链接的 no-replace 语义发布新文件后切换 Manifest；replace 还会先把旧媒体移到随机备份路径。普通异常会逆序清理或恢复，清理失败会与原错误一起以 `AggregateError` 报告。成功后才清理备份与日志。日志先于任何临时媒体写入，因此进程中断留下的 create 文件也有明确归属，不需要扫描或猜测孤儿。

`recover_project_assets` 是显式确定性恢复工具，不是 Agent 循环。它取得相同写锁后只接受单链接、非符号链接、32 KiB 内的 version 1 create/replace 日志；日志不包含绝对路径，临时/备份路径由经过 Schema 验证的 entry 与 transaction UUID 在项目内重新推导。当前 Manifest 的规范 SHA-256、revision 和完整目标 entry 必须精确匹配日志旧状态或新状态：未提交 create 会删除哈希匹配的新孤儿，未提交 replace 会恢复旧文件，已提交状态完成清理，其他状态原样保留并报错。删除任何媒体前还会按打开句柄、路径 inode/device、字节数和双 SHA-256 重新验证。`get_project_assets` 保持纯只读，不会暗中触发恢复。
