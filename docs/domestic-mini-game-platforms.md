...
生成器 0.10.0 已将 `gameforge-platform.json` 放入 Laya `assets/resources/`，官方构建后位于产物 `resources/`。第一版模板声明离线运行，网络、登录、分享、广告和支付全部关闭；静态校验器会把实际远程 URL 和常见 `tt.*` 用法与声明交叉核对。域名校验只能证明产物内 URL 使用 HTTPS、非本地/IP/端口且出现在声明中，不能证明开发者后台已配置合法域名或服务器满足 TLS 1.2，这两项仍属于账号级平台验收。
