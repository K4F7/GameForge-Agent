...
为首次资产创建和资产替换增加持久事务日志与显式 MCP 恢复，使进程在临时文件、新文件发布、旧文件备份、Manifest 切换或备份清理之间终止后，可以只依赖磁盘权威状态安全回滚或完成提交，不调用媒体 Provider。
...
9. create 日志先于任何临时媒体写入，旧 Manifest 状态只删除哈希精确匹配新 entry 的孤儿。
