...
1. 阅读 `AGENTS.md`、相关设计文档和即将修改的代码。
   同时读取 `config/model-routing.example.json` 作为模型角色/类别的默认路由建议。先以宿主实际 `models`/capability 输出确认可用性；未列出的模型不得声称已使用。Agent 路由仍由 CodeArts 负责，MCP 中的 spec/image/tts/sound 只执行单次确定性 Provider 操作。用户显式覆盖优先，其次按 primary/fallback，最后才使用宿主默认，并在实验记录中写实际生效模型。
...
3. `web` 默认使用 Phaser、Vite 与程序化占位素材；`douyin-mini-game` 使用生成器固定的 LayaAir 3.4.0 TypeScript 源工程。抖音 apply 后若 `build_douyin_mini_game` 已注册，只调用一次并原样记录 CLI 版本、包体与校验报告；工具内部固定构建目标、超时、日志上限和项目锁，不登录、预览、上传或发布。未注册时报告本地 CLI capability 缺失，不自行拼接 shell 命令。不得用 Chrome `verify_game_project` 冒充平台验收，DevTool/真机仍单独记录。仅在工具已注册且需求需要时：
