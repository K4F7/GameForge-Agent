...
3. `web` 默认使用 Phaser、Vite 与程序化占位素材；`douyin-mini-game` 使用生成器固定的 LayaAir 3.4.0 TypeScript 源工程。抖音 apply 后由 CodeArts 在项目外调用固定 `layaair build bytedancegame`，再调用 GameForge 静态校验；不得用 Chrome `verify_game_project` 冒充平台验收，DevTool/真机仍单独记录。仅在工具已注册且需求需要时：
