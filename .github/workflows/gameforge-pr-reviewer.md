...
- Treat a PR as ordinary documentation-only only when every changed file is a Markdown file outside every `AGENTS.md` and `CLAUDE.md` basename, `.github/**`, `.claude/**`, `.codex/**`, `.agents/**`, `.opencode/**`, `.codeartsdoer/skills/**`, and `.codeartsdoer/agents/**`.
